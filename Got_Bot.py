...
def _get_buckets():
    ...
    test_buckets = Prep_Data.gen_buckets('TEST_ids.enc', 'TEST_ids.dec')
    Data_buckets = Prep_Data.gen_buckets('TRAIN_ids.enc', 'TRAIN_ids.dec')
    train_bucket_sizes = [len(Data_buckets[b]) for b in range(len(Config_Params.BUCKETS))]
...
def _check_restore_parameters(sess, saver):
    """ Checkpoint restore """
    ckpt = tf.train.get_checkpoint_state(os.path.dirname(Config_Params.CPT_PATH + '/checkpoint'))
    if ckpt and ckpt.model_checkpoint_path:
        print("Loading parameters for the Chatbot")
        saver.restore(sess, ckpt.model_checkpoint_path)
    else:
        print("Initializing GOT-BOT")


def _eval_test_set(sess, model, test_buckets):
    """ Evaluate on the test set. """
    for bucket_id in range(len(Config_Params.BUCKETS)):
...
def train():
    ...
    model = BotModel(False, Config_Params.BATCH_SIZE)
    model.build_graph()
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            if iteration % skip_step == 0:
                ...
                start = time.time()
                total_loss = 0
                saver.save(sess, os.path.join(Config_Params.CPT_PATH, 'chatbot'), global_step=model.global_step)
...
def _find_right_bucket(length):
    ...
    return min([b for b in range(len(Config_Params.BUCKETS))
                if Config_Params.BUCKETS[b][0] >= length])
...
def _construct_response(output_logits, inv_dec_vocab):
    ...
    if Config_Params.EOS_ID in outputs:
        outputs = outputs[:outputs.index(Config_Params.EOS_ID)]
...
def chat():
    ...
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        _check_restore_parameters(sess, saver)
        output_file = open(os.path.join(Config_Params.PROCESSED_PATH, Config_Params.OUTPUT_FILE), 'a+')
...
def main():
    ...
    Prep_Data.make_dir(Config_Params.CPT_PATH)
